...
<template lang="">
    <div class="row row-cols-6 pt-4 px-2">
        <div v-for="(comic, index) in comics" :key="'comics-' + index" class="col my-3">
            <img :src="comic.thumb" />
            <p class="text-uppercase text-white mt-2">{{ comic.series }}</p>
        </div>
    </div>    
</template>
